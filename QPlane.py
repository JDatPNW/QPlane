...
env = QPlaneEnv(flightOrigin, flightDestinaion, n_actions, end, dictObservation, dictAction)
Q = QLearn(n_states, n_actions, gamma, lr, epsilon, decayRate, epsilonMin)


# prints out all metrics
def log(i_epoch, i_step, reward, state, actions_binary, observation, control, explore):
    print("\t\tGame ", i_epoch)
    print("\t\t\tMove ", i_step)
    print("\t\t\tState ", state)
...
def step(i_step, done, reward, oldObservation):
    ...
    return done, oldState, newState, action, actions_binary, oldObservation, newObservation, control, reward, explore
...
def epoch(i_epoch):
    ...
    for i_step in range(n_steps):
        done, oldState, newState, action, actions_binary, oldObservation, newObservation, control, reward, explore = step(i_step, done, reward, oldObservation)
        log(i_epoch, i_step, reward, oldState, actions_binary, oldObservation, control, explore)
        if done:
            break
